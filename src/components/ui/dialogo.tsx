...
export const CierreDialogo = DialogPrimitive.Close;
  ...
  <DialogPrimitive.Portal>
    ...
    <DialogPrimitive.Content
      ...
      className={cn(
        ...
        'max-h-[calc(100vh-4rem)] overflow-y-auto rounded-lg border border-border bg-card p-6 shadow-lg',
...
ContenidoDialogo.displayName = 'ContenidoDialogo';

export function TituloDialogo({ className, ...props }: React.ComponentPropsWithoutRef<typeof DialogPrimitive.Title>) {
  return <DialogPrimitive.Title className={cn('text-lg font-semibold', className)} {...props} />;
}

export function DescripcionDialogo({ className, ...props }: React.ComponentPropsWithoutRef<typeof DialogPrimitive.Description>) {
